...
//EnemyBoss Properties
[System.Serializable]
public class EnemyBoss 
{
	public GameObject enemyBossObj;		//Object Prefab
	public int Count;					//Number of the object in 1 wave
	public float StartWait;				//Time to Start spawning
}
...
{	
	...
	public EnemyBoss enemyBoss;         //make an Object from Class enemyBoss    
	...
	void Update () 
	{
		...
		if (appearBoss) {
			...
			StopCoroutine(enemyGreenSpawnWaves());
			StopCoroutine(enemyRedSpawnWaves());
		}


		if (SharedValues_Script.finish) {
			Score.text = "SCORE : " + SharedValues_Script.score;
	...
	IEnumerator enemyBossSpawn() {
		...
		yield return new WaitForSeconds (enemyBoss.StartWait);															//Wait for Seconds before start the wave
		
		Vector2 spawnPosition = new Vector2 (Random.Range (-spawnValues.x, spawnValues.x), spawnValues.y - 3);		//Random Spawn Position
		Quaternion spawnRotation = Quaternion.identity;	// Quaternion : can express rotation value														//Default Rotation
		Instantiate (enemyBoss.enemyBossObj, spawnPosition, spawnRotation);										//Instantiate Object
